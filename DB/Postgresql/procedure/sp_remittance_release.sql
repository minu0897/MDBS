...
CREATE OR REPLACE FUNCTION sp_remittance_release(
    p_idempotency_key VARCHAR(100),
    OUT p_status      VARCHAR(1),      -- '3': RELEASED, '2': ALREADY_CAPTURED, '0': NOT_FOUND
    OUT p_result      VARCHAR(32)      -- 'OK' | 'ALREADY_RELEASED' | 'ALREADY_CAPTURED' | 'HOLD_NOT_FOUND'
)
RETURNS RECORD AS $$
DECLARE
    v_account BIGINT;
...
BEGIN
    ...
    BEGIN
        SELECT account_id, amount, status
          INTO STRICT v_account, v_amount, v_hstat
          FROM holds
    ...
    -- 계좌 잠금
    PERFORM 1 FROM accounts WHERE account_id = v_account FOR UPDATE;
